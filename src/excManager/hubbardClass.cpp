...
namespace dftfe
{
        ...
        for (unsigned int i = 0; i < numberOfProjectors; i++)
          {
            ...
            d_atomicProjectorFnsMap[std::make_pair(Znum, alpha)] =
              std::make_shared<AtomCenteredPseudoWavefunctionSpline>(
                ...
                lQuantumNo,
                10.0, // the cut off is manually set to 10.0 to emulate QE's
                      // behaviour. Removie it if better accuracy is required
                1E-12);
            alpha++;
          } // i loop
    ...
    const std::vector<std::vector<double>> &domainBoundaries)
  {
    ...
    /*
            std::cout<<" rank = "<<thisRank<<"numTotalAtom =
       "<<numLocalAtomsInProc<<" numAtoms = "<<d_procLocalAtomId.size()<<"\n";
            for (unsigned int iAtom  = 0; iAtom < d_procLocalAtomId.size();
       iAtom++)
              {
                std::cout<<" iAtom = "<<iAtom<<" procLocal =
       "<<d_procLocalAtomId[iAtom]<<"\n";
              }
        */
    //    pcout<<" init var = "<<endVarInit-startInit<<" shape func =
    //    "<<endShape-endVarInit<<" read atom = "<<endRead-endShape<<" compute
    //    atom = "<<endAtom-endRead<<" disc atom = "<<endInit-endAtom<<"\n";
    ...
    double &hubbardEnergyCorrection)
  {
            ...
            for (unsigned int iOrb = 0; iOrb < numSphericalFunc; iOrb++)
              {
                ...
                hubbardEnergy -= 0.5 * d_spinPolarizedFactor *
                                 d_hubbardSpeciesData[hubbardIds].hubbardValue *
    ...
    MPI_Allreduce(
      MPI_IN_PLACE, &hubbardEnergy, 1, MPI_DOUBLE, MPI_SUM, d_mpi_comm_domain);
                ...
                if (true) /// TODO extend to band parallelisation
                  {
                    for (unsigned int iEigenVec = 0;
                         ...
                         ++iEigenVec)
                      {
                        ...
                        // pcout<<" iWave = "<<iEigenVec<<" orb occ in hubb =
                    ...
                    //                    iNode++)
                    //                      {
                    //                        flattenedArrayBlock->data()[iNode]
                    //                        = 1.0;
                    //                      }
    ...
    for (unsigned int iAtom = 0; iAtom < numOwnedAtomsInProc; iAtom++)
      {
        ...
        for (unsigned int spinIndex = 0; spinIndex < d_numSpins; spinIndex++)
          {
            std::cout << " occ number for atom = " << atomId
                      << " spin = " << spinIndex << "\n";
            for (unsigned int iOrb = 0; iOrb < numSphericalFunc; iOrb++)
              {
                for (unsigned int jOrb = 0; jOrb < numSphericalFunc; jOrb++)
    ...
    for (int iAtom = 0; iAtom < atomIdsInProcessor.size(); iAtom++)
      {
        ...
        //          {
        //            D_ij[isDijOut ? TypeOfField::Out :
        //            TypeOfField::In][atomId] =
        ...
        /*
           ...
           numberSphericalFunc*numberSphericalFunc; iWave++)
                  {
                    std::cout<<" iWave = "<<iWave<<" tempOccMat =
           "<<tempOccMat[iWave] <<"\n";
                  }
        */
  ...
  hubbard<ValueType, memorySpace>::computeCouplingMatrix()
  {
    ...
    // d_couplingMatrixEntries[0].size();iOrb++)
    //{
    //	    std::cout<<" iOrb = "<<iOrb <<" couple Val =
    //"<<d_couplingMatrixEntries[0].data()[iOrb]<<"\n";
    //}
  }
    ...
    if (d_nonLocalOperator->getTotalNonLocalElementsInCurrentProcessor() > 0)
      {
        ...
        // d_BasisOperatorMemPtr->distribute(src);
        for (unsigned int iCell = 0; iCell < nCells;
             iCell += d_cellsBlockSizeApply)
          {
            std::pair<unsigned int, unsigned int> cellRange(
  ...
  /*
  template <typename ValueType, dftfe::utils::MemorySpace memorySpace>
  void hubbard<ValueType, memorySpace>::readHubbardInput( const
  std::vector<std::vector<double>> &atomLocations, const std::vector<int>
  &imageIds, const std::vector<std::vector<double>> &imagePositions)
  {
    ...
    unsigned int numberOfSpecies;
    hubbardInputFile >> numberOfSpecies;
    d_noSpecies = numberOfSpecies - 1; // 0 is default species corresponding to
            ...
            for( int m = -l; m <=l;m++)
              {
                ...
                d_orbitalIds[id-1].push_back(orbitalNumObj); // I hope this is a
  copy operation
              }
    ...
    for(unsigned int imageIdIter = 0; imageIdIter<
  imageIds.size();imageIdIter++)
      {
        mapAtomToImageAtom[imageIds[imageIdIter]].push_back(imageIdIter);
      }
        ...
        if( id != 0)
          {
            ...
            for(unsigned int jImageAtom = 0; jImageAtom <
  mapAtomToImageAtom[iAtom].size();jImageAtom++)
              {
                d_speciesImageAtomList[id-1].push_back(mapAtomToImageAtom[iAtom][jImageAtom]);
                unsigned int speciesAtomId =
  mapAtomToImageAtom[iAtom][jImageAtom]; atomCoord[0] =
  imagePositions[speciesAtomId][0]; atomCoord[1] =
  imagePositions[speciesAtomId][1]; atomCoord[2] =
  imagePositions[speciesAtomId][2];
                d_atomicImageCoordinates[id-1].push_back(atomCoord);
                d_mapImageAtomToSpeciesAtomId[2*mapAtomToImageAtom[iAtom][jImageAtom]]
  = id-1; //species index
                d_mapImageAtomToSpeciesAtomId[2*mapAtomToImageAtom[iAtom][jImageAtom]
  + 1] = d_noAtoms[id-1] -1; // atom index

                for(unsigned int iKPoint = 0; iKPoint  <d_numKPoints; iKPoint++)
                  {
                    double angle = (atomLocations[iAtom][2] -
  imagePositions[speciesAtomId][0] )*d_kPointCoordinates[3*iKPoint+0] ; angle +=
  (atomLocations[iAtom][3] - imagePositions[speciesAtomId][1]
  )*d_kPointCoordinates[3*iKPoint+1] ; angle       += (atomLocations[iAtom][4] -
  imagePositions[speciesAtomId][2] )*d_kPointCoordinates[3*iKPoint+2] ;

                    copyComlexValueCompatibly(std::exp(std::complex<double>(0,-angle)),
  ...
  d_orbitalIds[iSpecies].size(); startIndexAtomicWavefunction[iSpecies] =
  d_totalAtomicWavefunction;
        //          d_noAtoms[iSpecies] = d_atomicCoordinates[iSpecies].size();
        d_totalAtomicWavefunction +=
  (d_noAtoms[iSpecies]*d_atomicWavefunctionPerSpecies[iSpecies]);
        d_atomicWavefunctionPerSpeciesSq[iSpecies] =
  d_atomicWavefunctionPerSpecies[iSpecies]*d_atomicWavefunctionPerSpecies[iSpecies];
  ...
  */
  //
  //  template <typename ValueType, dftfe::utils::MemorySpace memorySpace>
  //  void hubbard<ValueType, memorySpace>::computeAtomicData(const
  ...
  //      {
  //        cumulativeAtomicData[iSpecies] = cumulativeAtomicData[iSpecies-1] +
  //        d_noAtoms[iSpecies-1]*d_atomicWavefunctionPerSpecies[iSpecies-1];
  //      }
  //
  ...
  //    //std::fill(d_atomicOrbitalOverlapWithCell.begin(),d_atomicOrbitalOverlapWithCell.end(),false);
  ...
  //
  //        unsigned int totalAtomicWavePerSpecies =
  //        d_atomicWavefunctionPerSpecies[iSpecies]*
  //                                                 d_noAtoms[iSpecies];
  //        d_atomicWaveFuncStart[iSpecies][0] = 0;
  //        d_atomicWaveFuncEnd[iSpecies][0] =
  //        d_atomicWaveFuncStart[iSpecies][0]
  //                                           +
  //                                           totalAtomicWavePerSpecies/n_mpi_processes;
  //        if(totalAtomicWavePerSpecies%n_mpi_processes >  0)
  //          {
  //            d_atomicWaveFuncEnd[iSpecies][0] =
  //            d_atomicWaveFuncEnd[iSpecies][0] +1;
  //          }
  //        for(unsigned int iProc = 1; iProc <n_mpi_processes; iProc++)
  //          {
  //            d_atomicWaveFuncStart[iSpecies][iProc] =
  //            d_atomicWaveFuncEnd[iSpecies][iProc-1];
  //            d_atomicWaveFuncEnd[iSpecies][iProc] =
  //            d_atomicWaveFuncStart[iSpecies][iProc]
  //                                                   +
  //                                                   totalAtomicWavePerSpecies/n_mpi_processes
  //                                                   ;
  ...
  //
  //        AtomicOrbitalBasisManager atomicOrbitalObj(
  //        d_atomicNumber[iSpecies], // atomic number how do you get this ?
  //                                                   false, // normalised ?
  //                                                   d_dftParamsPtr,
  ...
  //          {
  //            std::cerr << " Local range is not contiguous " <<
  //            localIndex.is_contiguous()
  //                      << std::endl;
  //          }
  //
  //        // TODO extend it to k points
  //        for( unsigned int orbitalId = 0 ; orbitalId <
  //        d_atomicWavefunctionPerSpecies[iSpecies]; orbitalId++)
  //          {
  //            initialOccupation[iSpecies][orbitalId] =
  //            0.5*atomicOrbitalObj.getOccupationForOrbital(
  ...
  //        //                  {
  //        //                    unsigned int atomId =
  //        iWave/d_atomicWavefunctionPerSpecies[iSpecies];
  //        //                    unsigned int orbitalId =
  //        iWave%d_atomicWavefunctionPerSpecies[iSpecies];
  //        //
  //        atomicWaveNodalData[iSpecies].data()[localNode*d_atomicWavefunctionPerSpecies[iSpecies]*d_noAtoms[iSpecies]
  //        + iWave] =
  ...
  //
  //        dealii::QGauss<3>
  //        quadratureRhs(d_dftParamsPtr->finiteElementPolynomialOrder + 1);
  //
  ...
  //              for(unsigned int iAtom = 0; iAtom <d_noAtoms[iSpecies];
  //              iAtom++)
  //                {
  //                  double atomicDist =
  ...
  //              minDistAtom = 10000;
  //              for(unsigned int iAtom = 0; iAtom
  //              <d_speciesImageAtomList[iSpecies].size(); iAtom++)
  //                {
  //                  double atomicDist = (center[0] -
  //                  d_atomicImageCoordinates[iSpecies][iAtom][0])*(center[0] -
  //                  d_atomicImageCoordinates[iSpecies][iAtom][0]) +
  //                                      (center[1] -
  //                                      d_atomicImageCoordinates[iSpecies][iAtom][1])*(center[1]
  //                                      -
  //                                      d_atomicImageCoordinates[iSpecies][iAtom][1])
  //                                      + (center[2] -
  //                                      d_atomicImageCoordinates[iSpecies][iAtom][2])*(center[2]
  //                                      -
  //                                      d_atomicImageCoordinates[iSpecies][iAtom][2]);
  //
  //                  atomicDist = std::sqrt(atomicDist);
  ...
  //              /*
  //                            for( unsigned int waveId = 0 ; waveId <
  //                            d_cellToNumAtomicOrbitalOverlap[iSpecies][iElem];
  //                            waveId++)
  //                              {
  //                                unsigned int iWave =
  //                                d_cellToAtomicOrbitalOverlap[iSpecies][iElem][waveId];
  //                                unsigned int atomId =
  //                                iWave/d_atomicWavefunctionPerSpecies[iSpecies];
  //                                unsigned int orbitalId =
  //                                iWave%d_atomicWavefunctionPerSpecies[iSpecies];
  ...
  //
  //                                    atomicWaveCellWiseQuadDataJxWOpt[iSpecies][iElem][waveId*d_numberQuadraturePoints
  //                                    + qPoint]
  //                                      =
  //                                      atomicOrbitalObj.PseudoAtomicOrbitalvalue(
  //                                          d_orbitalIds[iSpecies][orbitalId],
  ...
  //                                unsigned int orbitalId =
  //                                iWave%d_atomicWavefunctionPerSpecies[iSpecies];
  //                                elemWaveQPointContiguousIndex =
  //                                elemQuadIndex +
  //                                iWave*d_numberQuadraturePoints; for
  //                                (unsigned int qPoint = 0; qPoint <
  //                                d_numberQuadraturePoints;
  //                                     qPoint++)
  //                                  {
  //                                    dealii::Point<3, double> qPointVal =
  //                                      fe_values.quadrature_point(qPoint);
  //                                    unsigned int
  //                                    atomicWaveQuadDataJxWOptIndex =
  //                                    elemWaveQPointContiguousIndex + qPoint;
  //
  ...
  //                  for(unsigned int iKPoint = 0; iKPoint  <d_numKPoints;
  //                  iKPoint++)
  //                    {
  //                      double angle = d_kPointCoordinates[3*iKPoint
  //                      +0]*qPointVal[0]; angle       +=
  //                      d_kPointCoordinates[3*iKPoint +1]*qPointVal[1]; angle
  //                      += d_kPointCoordinates[3*iKPoint +2]*qPointVal[2];
  //                      copyComlexValueCompatibly(std::exp(std::complex<double>(0,-angle)),phaseFactorQuadPoint[iKPoint]);
  //                    }
  //
  //                  for(unsigned int atomIndex =0; atomIndex <
  //                  atomListInCell.size();atomIndex++)
  //                    {
  ...
  //                      double r{}, theta{}, phi{};
  //
  //                      auto relativeEvalPoint = relativeVector3d(qPointVal,
  //                      d_atomicCoordinates[iSpecies][atomId]);
  //
  //                      convertCartesianToSpherical(relativeEvalPoint, r,
  //                      theta, phi);
  //
  //                      double radialPart =
  //                      atomicOrbitalObj.RadialPseudoAtomicOrbital(n, l, r);
  //
  //                      for(unsigned int orbitalId = 0;
  //                      orbitalId<d_atomicWavefunctionPerSpecies[iSpecies];
  //                      orbitalId++)
  //                        {
  //                          m =
  //                          d_orbitalIds[iSpecies][orbitalId].mOrbitalNumber;
  //                          quadIndex = elemQPointIndex +
  //                          atomId*d_atomicWavefunctionPerSpecies[iSpecies]+orbitalId;
  //                          double harmonicPart =
  //                          atomicOrbitalObj.realSphericalHarmonics(l, m,
  //                          theta, phi);
  //
  //                          double atomicWaveFunctionVal =
  //                          radialPart*harmonicPart; for(unsigned int iKPoint
  //                          = 0; iKPoint  <d_numKPoints; iKPoint++)
  //                            {
  ...
  //
  //                  for(unsigned int atomIndex =0; atomIndex <
  //                  imageAtomListInCell.size();atomIndex++)
  //                    {
  //                      unsigned int atomId =
  //                      d_mapImageAtomToSpeciesAtomId[2*imageAtomListInCell[d_speciesImageAtomList[iSpecies][atomIndex]]
  //                      + 1]; // atom id unsigned int speciesAtomId =
  //                      imageAtomListInCell[d_speciesImageAtomList[iSpecies][atomIndex]];
  //
  //                      int n = d_orbitalIds[iSpecies][0].nOrbitalNumber;
  //                      int l = d_orbitalIds[iSpecies][0].lOrbitalNumber;
  //                      int m = d_orbitalIds[iSpecies][0].mOrbitalNumber;
  ...
  //
  //                          double atomicWaveFunctionVal =
  //                          radialPart*harmonicPart;
  //
  //                          quadIndex = elemQPointIndex +
  //                          atomId*d_atomicWavefunctionPerSpecies[iSpecies]+orbitalId;
  //                          for(unsigned int iKPoint = 0; iKPoint
  //                          <d_numKPoints; iKPoint++)
  //                            {
  //                              atomicWaveQuadDataJxW[iKPoint][iSpecies][quadIndex]
  //                              +=
  //                                atomicWaveFunctionVal *
  ...
  //  void hubbard<ValueType, memorySpace>::computeSymmetricTransforms(const
  ...
  //
  //    int dimIn = 5;
  //    linearAlgebraOperations::inverse(&sphericalHarmonicsInitialInv[0],
  //    dimIn);
  //
  ...
  //  // It is expected that it will be called in the kohnShamOperator.cc after
  //  this
  //  // function is called
  //  template <typename ValueType, dftfe::utils::MemorySpace memorySpace>
  //  void hubbard<ValueType,
  //  memorySpace>::preComputeDiscreteAtomicWavefunctions()
  //  {
  //    const ValueType scalarCoeffAlpha = ValueType(1.0);
  ...
  //        for (unsigned int iSpecies = 0 ; iSpecies < d_noSpecies ;
  //        iSpecies++)
  //          {
  //            iElem = 0 ;
  //            unsigned int totalAtomicWaveFunctions =
  //            d_atomicWavefunctionPerSpecies[iSpecies]*d_noAtoms[iSpecies];
  //
  //
  //            dftfe::linearAlgebra::createMultiVectorFromDealiiPartitioner(
  ...
  //              (d_numberDofsPerElement*d_numberQuadraturePoints,
  //              ValueType(0.0));
  //
  //            for (unsigned int iquad = 0; iquad < d_numberQuadraturePoints;
  //            ++iquad)
  //              for (unsigned int iNode = 0; iNode < d_numberDofsPerElement;
  //              ++iNode)
  //                shapeFunctionValues[iquad + iNode*d_numberQuadraturePoints]
  //                =
  //                  ValueType(d_shapeFunctionValues[iquad *
  //                  d_numberDofsPerElement + iNode]);
  //
  ...
  //                    //                           &inc,
  //                    // &cellLevelAtomicWave[totalAtomicWaveFunctions *
  //                    iNode],
  //                    //                           &inc);
  //                    //                  }
  //                    //
  ...
  //
  //                    //for (unsigned int dofId = 0; dofId <
  //                    d_numberDofsPerElement; dofId++)
  //                    //  {
  //                    //    for (unsigned int q_point = 0; q_point <
  //                    d_numberQuadraturePoints ; q_point++)
  //                    //      {
  //                    //        for (unsigned int inputId = 0; inputId <
  //                    totalAtomicWaveFunctions; inputId++)
  //                    //          {
  //                    //            cellLevelDiscreteAtomicWavefunc[dofId *
  //                    totalAtomicWaveFunctions + inputId ] +=
  //                    //
  ...
  //                    //              d_shapeFunctionValues[dofId +
  //                    q_point*d_numberDofsPerElement];
  //                    //
  //                    //                          }
  //                    //                      }
  //                    //
  ...
  //                             &scalarCoeffAlpha,
  //                             &cellLevelDiscreteAtomicWavefunc[totalAtomicWaveFunctions
  //                             * iNode], &inc,
  ...
  //              }
  //
  //            //constraintsMatrixSpecies.distribute_slave_to_master(atomicDiscreteData[iSpecies],
  //            totalAtomicWaveFunctions);
  //            atomicDiscreteData[iKPoint][iSpecies].accumulateAddLocallyOwned();
  //            //
  //            constraintsMatrixSpecies.distribute(atomicDiscreteData[iSpecies],
  //            totalAtomicWaveFunctions);
  //
  //            /*
  ...
  //  {
  //    return
  //    d_atomicProjectorFnsContainer->getAtomIdsInCurrentProcess().size();
  //  }
