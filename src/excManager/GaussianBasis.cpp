...
namespace dftfe
{
  ...
  namespace
  {
    ...
    void printContractedGaussian(const ContractedGaussian & cg)
    {
      const int nC = cg.nC;
      for(unsigned int i = 0; i < nC; ++i)
      {
        std::cout << "alpha: " << cg.alpha[i] << " c: " << cg.c[i] << " norm: " << cg.norm[i] << std::endl;
      }
    }
    ...
    double getLimitingValueLaplacian(const int l, const int m, const double theta, const double angleTol)
    {
        ...
        const double angleTol)
    {
      ...
      const int m = cg->m;
      double r, theta, phi;
      utils::sphUtils::convertCartesianToSpherical(x, r, theta, phi, rTol, angleTol);
      double returnValue = 0.0;
      for(unsigned int i = 0; i < nC; ++i)
      {
        ...
        double angleTol)
    {
      ...
      {
        std::vector<std::vector<double>> jacobianInverse = utils::sphUtils::getJInv(r, theta, phi);
    ...
    double getContractedGaussianLaplacian(const ContractedGaussian * cg, const std::vector<double> & x, double rTol, double angleTol)
    {
      ...
      const std::vector<double> dYlm = utils::sphUtils::dYlmReal(l, m, theta, phi); 
      const std::vector<double> d2Ylm = utils::sphUtils::d2YlmReal(l, m, theta, phi); 
      double R = 0.0;
      double dRdr = 0.0;
      double d2Rdr2 = 0.0;
      double S = 0.0;
      for(unsigned int i = 0; i < nC; ++i)
      {
        ...
        R += cVal*norm*gaussianRadialPart(r, l, alphaVal);
        dRdr += cVal*norm*gaussianRadialPartDerivative(r, alphaVal, l, 1);
        d2Rdr2 += cVal*norm*gaussianRadialPartDerivative(r, alphaVal, l, 2);
      ...
      else
      {
        ...
        {
          const double limitingVal = getLimitingValueLaplacian(l, modM, theta, angleTol);
        ...
        {
          const double term2 = (R/(r*r)) * ((cosTheta/sinTheta)*dYlm[0] + d2Ylm[0]);
          const double term3 = (R/(r*r))*d2Ylm[1]/(sinTheta*sinTheta);
          returnValue = term1 + term2 + term3;
        }
      }
      return returnValue;
    ...
    void
      ...
      {
        std::unordered_map<char, int> lCharToIntMap = {
        ...
        while(std::getline(readFile,readLine))
        {
          ...
          std::vector<std::string> words = utils::stringOps::split(lineString.str(), "\t ");
          ...
          msg = "Undefined L character(s) for the contracted Gaussian read in file " + basisFileName;
          ...
          msg =  "Undefined number of contracted Gaussian in file " + basisFileName;
          utils::throwException(isInt, msg);
          std::vector<double> alpha(nContracted,0.0);
          std::vector<std::vector<double>> c(numLChars, std::vector<double>(nContracted,0.0));
            ...
            if(std::getline(readFile,readLine))
            {
              ...
              for(unsigned int j = 0; j < numLChars; ++j)
              {
                ...
                std::string msg= "Undefined value " + coeffStr + 
                  ...
                  basisFileName;
                bool isNumber = utils::stringOps::strToDouble(coeffStr, c[j][i]);
                utils::throwException(isNumber,msg);
              }
            }
            else
          ...
          for(unsigned int j = 0; j < numLChars; ++j)
          {
            ...
            {
              std::string msg = "Character " + std::string(1,lChar) + " does not exist in the "
      ...
      const std::unordered_map<std::string, std::string> & atomBasisFileName)
  {
    ...
    for (const auto &pair : atomBasisFileName)
    {
      ...
      d_atomToContractedGaussiansPtr[atomSymbol] = std::vector<ContractedGaussian *>(0);
  ...
  std::vector<double> 
        ...
        const std::vector<double> & x) const
    {
      ...
      for(unsigned int iPoint = 0; iPoint < nPoints; ++iPoint)
      {
        ...
        returnValue[iPoint] = getContractedGaussianValue(cg, dx, d_rTol, d_angleTol);
      ...
      const std::vector<double> & x) const
  {
    ...
    for(unsigned int iPoint = 0; iPoint < nPoints; ++iPoint)
    {
      ...
      std::vector<double> tmp = getContractedGaussianGradient(cg, dx, d_rTol, d_angleTol);
      for(unsigned int j = 0; j < 3; ++j)
  ...
  std::vector<double> GaussianBasis::getBasisLaplacian(const unsigned int basisId, 
      const std::vector<double> & x) const
  {
    const GaussianBasisInfo & info = d_gaussianBasisInfo[basisId];
    const double * x0 = info.center;
    ...
    for(unsigned int iPoint = 0; iPoint < nPoints; ++iPoint)
    {
      ...
      returnValue[iPoint] = getContractedGaussianLaplacian(cg, dx, d_rTol, d_angleTol);
    }
    return returnValue;
  }
}
