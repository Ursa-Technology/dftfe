		...
		int nQuad  = quadpts.size() / 3;
      	...
      	switch (maxDerOrder)
      	{
      		...
      		case 1:
      			...
      			{
      				auto basisVals = atomicBasis.getBasisValue(iBasis, quadpts);
      				auto basisGradVals = atomicBasis.getBasisGradient(iBasis, quadpts);
      				...
      				for (unsigned int iQuad = 0; iQuad < nQuad; iQuad++)
			        {
			          ...
			          {
			          	d_basisGradValues[iQuad * nBasis * 3 + iBasis * 3 + iDim] = basisGradVals[iQuad * 3 + iDim];
			          }
			        }
      			}
      			break;
      		...
      		case 2:
      			...
      			for (unsigned int iBasis = 0; iBasis < nBasis; iBasis++)
      			{
      				auto basisVals = atomicBasis.getBasisValue(iBasis, quadpts);
      				auto basisGradVals = atomicBasis.getBasisGradient(iBasis, quadpts);
      				auto basisLapVals = atomicBasis.getBasisLaplacian(iBasis, quadpts);
