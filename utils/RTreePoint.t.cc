...
namespace dftfe
{
  namespace utils
  {
    namespace
    {
      ...
      void assignValueToBoostPoint(boost::geometry::model::point<double, dim, BG::cs::cartesian> &p,
                              const std::vector<double> &vec)
      {
        throwException(false, "Dim of the Boost point is not extended to the current value \n");
      }

      template<>
      void assignValueToBoostPoint<3>(boost::geometry::model::point<double, 3, BG::cs::cartesian> &p,
                                 const std::vector<double> &vec)
      {
      ...
      template<>
      void assignValueToBoostPoint<2>(boost::geometry::model::point<double, 2, BG::cs::cartesian> &p,
                                 const std::vector<double> &vec)
      {
      ...
      template<>
      void assignValueToBoostPoint<1>(boost::geometry::model::point<double, 1, BG::cs::cartesian> &p,
                                 const std::vector<double> &vec)
      {
        p.set<0>(vec[0]);
      }
        ...
        namespace RTreePointInternal
    {
                    ...
                    const std::vector<double> & ur)
      {
        ...
        return BG::model::box<BG::model::point<double, dim, BG::cs::cartesian>>(pointLowerLeft, pointUpperRight);
      }
    }

        template <size_type dim, size_type M>
    RTreePoint<dim,M>::RTreePoint(const std::vector<std::vector<double>> & srcPts)
    {
      const size_type nPts = srcPts.size();
      std::vector<std::pair<BG::model::point<double, dim, BG::cs::cartesian>, size_type>> srcPtsI(nPts);
      ...
      d_rtreePtr= std::make_shared<BGI::rtree<std::pair<BG::model::point<double, dim, BG::cs::cartesian>, size_type>, BGI::quadratic<M>>>(srcPtsI.begin(), srcPtsI.end());
    }

    template <size_type dim, size_type M>
    std::vector<size_type>
    RTreePoint<dim,M>::getPointIdsInsideBox(const std::vector<double> & lowerLeft,
                                             const std::vector<double> & upperRight)
    {
      std::vector<std::pair<BG::model::point<double, dim, BG::cs::cartesian>, size_type>> foundPointI(0);
