  ...
  for (int iQuad = 0; iQuad < d_nQuad; iQuad++)
    {
      ...
      sigma[0]    = gradrhoUp[0] * gradrhoUp[0] + gradrhoUp[1] * gradrhoUp[1] +
                 gradrhoUp[2] * gradrhoUp[2];
      sigma[1] = gradrhoUp[0] * gradrhoDown[0] + gradrhoUp[1] * gradrhoDown[1] +
                 gradrhoUp[2] * gradrhoDown[2];
      sigma[2] = gradrhoDown[0] * gradrhoDown[0] +
                 gradrhoDown[1] * gradrhoDown[1] +
                 gradrhoDown[2] * gradrhoDown[2];
      ...
      this->setLocalDescriptors(DensityDescriptorDataAttributes::valuesSpinUp,
                                indexRange,
                                rhoUp);
      this->setLocalDescriptors(DensityDescriptorDataAttributes::valuesSpinDown,
                                indexRange,
                                rhoDown);
      this->setLocalDescriptors(DensityDescriptorDataAttributes::valuesTotal,
                                indexRange,
                                rhoTotal);
      this->setLocalDescriptors(
      ...
      this->setLocalDescriptors(
        ...
        indexRange,
        gradrhoDown);
      this->setLocalDescriptors(DensityDescriptorDataAttributes::sigma,
                                indexRange,
                                sigma);

      indexRange = std::make_pair(iQuad * 9, iQuad * 9 + 9);
      this->setLocalDescriptors(DensityDescriptorDataAttributes::hessianSpinUp,
                                indexRange,
                                HessianrhoUp);
      this->setLocalDescriptors(
                                             ...
                                             double                     beta)
{
  ...
  d_maxDerOrder); d_sbd.evalSlaterOverlapMatrixInv(d_quadWt, d_nQuad, d_nBasis);
  }
