...
namespace dftfe
{
  namespace utils
  {
    ...
    Cell<dim>::Cell(
      const std::vector<double> &ll, const std::vector<double> &ur)
    {
                             ...
                             const double tol) const
    {
      ...
      for( unsigned int j = 0 ; j< dim; j++)
        {
          if((point[j] < d_lowerLeft[j] - tol) || (point[j] > d_upperRight[j] + tol))
            {
              returnVal = false;
            }
        }
    ...
    Cell<dim>::getParametricPoint(const std::vector<double> &realPoint)const
    {
      ...
      for(unsigned int j = 0 ; j <dim; j++)
        {
          pointParam[j] = (realPoint[j] - d_lowerLeft[j]) /(d_upperRight[j] - d_lowerLeft[j]);
        }
