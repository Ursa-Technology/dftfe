  ...
  void
        ...
        const std::unordered_map<std::string, std::string> & atomBasisFileNames)
    {
      ...
      for (const auto &pair : atomBasisFileNames)
      {
        ...
        d_atomToSlaterPrimitivesPtr[atomSymbol] = std::vector<SlaterPrimitive *>(0);
        getSlaterPrimitivesFromBasisFile(basisName, d_atomToSlaterPrimitivesPtr[atomSymbol]);
      }
      ...
      for (unsigned int i = 0; i < natoms; ++i)
      {
        ...
        unsigned int nprimitives = d_atomToSlaterPrimitivesPtr[atomSymbol].size();
