...
#define DFTFE_FINITE_DIFF_H 1e-3
  namespace
  {
    ...
    double
    evaluateBasisValue(const dftfe::gaussianFunctionManager::basis *b,
                       const double *                               x)
    {
      const int     L = b->L;
    ...
    getBasisHigherOrderDerivativeFD(
      const double *                               point,
      const dftfe::gaussianFunctionManager::basis *basis,
    ...
    void
    readGaussianFiles(
      std::vector<dftfe::gaussianFunctionManager::contractedGaussian *>
                  ...
                  if (isInteger(nContracted, strNContracted))
                    {
                      ...
                      for (unsigned int j = 0; j < numLChars; ++j)
                        {
                          dftfe::gaussianFunctionManager::contractedGaussian
                            *cg = new dftfe::gaussianFunctionManager::
                              contractedGaussian;
                          cg->L        = nContracted;
    ...
    void
    storeBasisFunctions(
      std::vector<dftfe::gaussianFunctionManager::basis *> &basisFunctions,
      const std::vector<
        std::vector<dftfe::gaussianFunctionManager::contractedGaussian *>>
                                             &                                     contractedGaussians,
      ...
      for (unsigned int iAtom = 0; iAtom < numAtoms; ++iAtom)
        {
          ...
          for (unsigned int j = 0; j < numContractedGaussians; ++j)
            {
              const dftfe::gaussianFunctionManager::contractedGaussian *cg =
              ...
              if (polyOrder == 1)
                {
                  ...
                  for (unsigned int iM = 0; iM < 3; ++iM)
                    {
                      dftfe::gaussianFunctionManager::basis *b =
                        new dftfe::gaussianFunctionManager::basis;
                      b->L     = nContracted;
                      b->alpha = cg->alpha;
                      b->c     = cg->c;
                      b->l     = polyOrder;
    ...
    void
    getBasisCutoff(const std::vector<dftfe::gaussianFunctionManager::basis *>
                                       &                  basisFunctions,
                   ...
                   const double         tol)
    {
      ...
      for (unsigned int i = 0; i < numBasis; ++i)
        {
          dftfe::gaussianFunctionManager::basis *b    = basisFunctions[i];
          int                                    L    = b->L;
          double                                 rcut = 0.0;
    ...
    getClosestDistanceToBasis(const double *     points,
                              ...
                              const dftfe::gaussianFunctionManager::basis *b)
    {
      double        rmin = 1e15;
      const double *R    = b->origin;
    ...
    printData(const std::string filename,
              const std::vector<dftfe::gaussianFunctionManager::basis *>
                                                                  &                                     basisFunctions,
  ...
  gaussianFunctionManager::~gaussianFunctionManager()
  {
    ...
    for (unsigned int i = 0; i < numAtoms; ++i)
      {
        ...
        for (unsigned int j = 0; j < numAtomicContractedGaussians; ++j)
          {
            dftfe::gaussianFunctionManager::contractedGaussian *cg =
              atomicContractedGaussians[j];
            delete cg->alpha;
            delete cg->c;
            delete cg;
    ...
    for (unsigned int i = 0; i < numBasis; ++i)
      {
        dftfe::gaussianFunctionManager::basis *b = d_basisFunctions[i];
        delete b->normConsts;
        delete b;
      }
  }
  ...
  gaussianFunctionManager::getBasisFunctionGradient(const int     basisId,
                                                    const double *x) const
  {
    const dftfe::gaussianFunctionManager::basis *b = d_basisFunctions[basisId];
    const int                                    L = b->L;
    const double *                               R = b->origin;
    const int                                    l = b->l;
    const int                                    m = b->m;
  ...
  //	gaussianFunctionManager::getEvaluatedSMat()
  //	{
  //
  //	  int meshId = 0;
  //
  ...
  // fieldQuadratureTypeManager.getFieldQuadratureType(dftfe::ArrayNameManager::RHO);
  //
  //	  //
  ...
  // std::vector<double>(numBasis)); 	  for(unsigned int i = 0; i < numBasis;
  // ++i)
  //	  {
  //		for(unsigned int j = 0; j < numBasis; ++j)
  //		{
  ...
  // quadValues.getElementValues(iElem);
  //
  //			//
  ...
  //		  }
  //
  //		  SMat[i][j] = dftfe::FieldIntegrator().integrate(quadValues,
  //			  meshId);
  //		}
  //	  }
  //
