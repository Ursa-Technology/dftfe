...
namespace dftfe
{
            ...
            std::map<xcOutputDataAttributes, std::vector<double>> &cDataout) const;
    {
        ...
        std::map<DensityDescriptorDataAttributes, std::vector<double>> densityDescriptorData;
        ...
        for (size_type i = 0; i < d_densityDescriptorAttributesList.size(); i++)
        {
            ...
            else if (d_densityDescriptorAttributesList[i] = DensityDescriptorDataAttributes::sigma)
            densityDescriptorData[d_densityDescriptorAttributesList[i]] =
        ...
        auto &densityValuesSpinUp =
                densityDescriptorData.find(DensityDescriptorDataAttributes::valuesSpinUp)->second;
        auto &densityValuesSpinDown =
                densityDescriptorData.find(DensityDescriptorDataAttributes::valuesSpinDown)->second;
        auto &gradValuesSpinUp =
                densityDescriptorData.find(DensityDescriptorDataAttributes::gradValueSpinUp)->second;
        auto &gradValuesSpinDown =
                densityDescriptorData.find(DensityDescriptorDataAttributes::gradValueSpinDown)->second;
        auto &laplacianValuesSpinUp =
                densityDescriptorData.find(DensityDescriptorDataAttributes::laplacianSpinUp)->second;
        auto &laplacianValuesSpinDown =
                densityDescriptorData.find(DensityDescriptorDataAttributes::laplacianSpinDown)->second;
        ...
        for (size_type i = 0; i < quadGrid.getLocalSize(); i++)
        {
            ...
            for (size_type j = 0; j < 3; j++) {
                sigmaValues[3 * i + 0] += gradValuesSpinUp[3 * i + j] * gradValuesSpinUp[3 * i + j];
                sigmaValues[3 * i + 1] += gradValuesSpinUp[3 * i + j] * gradValuesSpinDown[3 * i + j];
                sigmaValues[3 * i + 2] += gradValuesSpinDown[3 * i + j] * gradValuesSpinDown[3 * i + j];
            }
        ...
        if (d_NNLLMGGAPtr != nullptr)
                {
                    ...
                    const size_type     numDescriptors = d_densityDescriptorAttributesList.size();
        ...
        if (isVxcBeingComputed)
        {
            ...
            const std::vector<double> stencil1D =
                    utils::FiniteDifference::
                    getStencilGridOneVariableCentral(d_vxcDivergenceTermFDStencilSize,
                                                     d_spacingFDStencil);

            std::map<DensityDescriptorDataAttributes, std::vector<double>> densityDescriptorDataForFD;
            ...
            std::vector<double> pdexDensityValuesNonNNFD(2 *
                                                         quadGrid.getLocalSize(), 0); // not used
            ...
            std::vector<double> pdexLaplacianValuesFD(2 * quadGrid.getLocalSize(), 0);
            std::vector<double> pdecLaplacianValuesFD(2 * quadGrid.getLocalSize(), 0); // not used



            for (size_type idim = 0; idim < 3; idim++)
            {
                for (size_type istencil = 0; istencil < d_vxcDivergenceTermFDStencilSize; istencil++)
                {
                    ...
                    for (size_type igrid = 0; igrid < quadGrid.getLocalSize(); igrid++)
                    {
                        ...
                        quadShiftedFD[3 * igrid + idim] =
                                ...
                                (-std::floor(d_vxcDivergenceTermFDStencilSize / 2) * hSizeFD[igrid] +
                                igrid * hSizeFD[igrid]);
                    }

                    auxDensityMatrix.applyLocalOperations(quadShiftedFD, densityDescriptorDataForFD);

                    auto &densityValuesSpinUpFD =
                            densityDescriptorDataForFD.find(DensityDescriptorDataAttributes::valuesSpinUp)
                                    ->second;
                    auto &densityValuesSpinDownFD =
                            densityDescriptorDataForFD.find(DensityDescriptorDataAttributes::valuesSpinDown)
                                    ->second;
                    auto &gradValuesSpinUpFD =
                            densityDescriptorDataForFD.find(DensityDescriptorDataAttributes::gradValueSpinUp)
                                    ->second;
                    auto &gradValuesSpinDownFD =
                            densityDescriptorDataForFD.find(DensityDescriptorDataAttributes::
                                                            gradValueSpinDown)->second;
                    auto &laplacianValuesSpinUpFD =
                            densityDescriptorDataForFD.find(DensityDescriptorDataAttributes::laplacianSpinUp)
                                    ->second;
                    auto &laplacianValuesSpinDownFD =
                            densityDescriptorDataForFD.find(DensityDescriptorDataAttributes::
                                                                laplacianSpinDown)->second;
                    ...
                    for (size_type i = 0; i < quadGrid.getLocalSize(); i++)
                    {
                        ...
                        for (size_type j = 0; j < 3; j++) {
                            sigmaValuesFD[3 * i + 0] += gradValuesSpinUpFD[3 * i + j] *
                            gradValuesSpinUpFD[3 * i + j];
                            sigmaValuesFD[3 * i + 1] += gradValuesSpinUpFD[3 * i + j] *
                            gradValuesSpinDownFD[3 * i + j];
                            sigmaValuesFD[3 * i + 2] += gradValuesSpinDownFD[3 * i + j] *
                            gradValuesSpinDownFD[3 * i + j];
                        }
                    ...
                    if (d_NNLLMGGAPtr != nullptr)
                    {
                        ...
                        std::vector<double> excValuesFromNNFD(quadGrid.getLocalSize(), 0);
                        const size_type numDescriptors = d_densityDescriptorAttributesList.size();
                        std::vector<double> pdexcDescriptorValuesFromNNFD(numDescriptors *
                                                                        quadGrid.getLocalSize(), 0);


                        d_NNLLMGGAPtr->evaluatevxc(&(densityValuesFD[0]),
                                                   &sigmaValuesFD[0],
                                                   &laplacianValuesFD[0],
                                                   quadGrid.getLocalSize(),
                                                   &excValuesFromNNFD[0],
                    ...
                    for (size_type igrid = 0; igrid < quadGrid.getLocalSize(); igrid++)
                    {
                        ...
                        [igrid * d_vxcDivergenceTermFDStencilSize + istencil] =
                            ...
                            (gradValuesSpinUpFD[idim * quadGrid.getLocalSize() + igrid]
                            + gradValuesSpinDownFD[idim * quadGrid.getLocalSize() + igrid]);
