...
namespace dftfe
{
                                 ...
                                 std::vector<double> &         vxc)
  {
    ...
    for (int iPoint = 0; iPoint < nPoints; iPoint++)
      {
        auto valuesSpinUp = auxDM.getLocalDescriptors(
        ...
        auto valuesSpinDown = auxDM.getLocalDescriptors(
          DensityDescriptorDataAttributes::valuesSpinDown,
          std::make_pair(iPoint, iPoint + 1));
        rho.insert(rho.end(), valuesSpinDown.begin(), valuesSpinDown.end());

        auto gradValuesSpinUp = auxDM.getLocalDescriptors(
          DensityDescriptorDataAttributes::gradValueSpinUp,
          std::make_pair(iPoint * 3, iPoint * 3 + 3));
        drho.insert(drho.end(),
                    gradValuesSpinUp.begin(),
                    gradValuesSpinUp.end());

        auto gradValuesSpinDown = auxDM.getLocalDescriptors(
          DensityDescriptorDataAttributes::gradValueSpinDown,
          std::make_pair(iPoint * 3, iPoint * 3 + 3));
        drho.insert(drho.end(),
                    gradValuesSpinDown.begin(),
                    gradValuesSpinDown.end());

        auto sigmaValues =
          auxDM.getLocalDescriptors(DensityDescriptorDataAttributes::sigma,
                                    std::make_pair(iPoint * 3, iPoint * 3 + 3));
        sigma.insert(sigma.end(), sigmaValues.begin(), sigmaValues.end());

        auto hessianSpinUp = auxDM.getLocalDescriptors(
          DensityDescriptorDataAttributes::hessianSpinUp,
          std::make_pair(iPoint * 9, iPoint * 9 + 9));
        ddrho.insert(ddrho.end(), hessianSpinUp.begin(), hessianSpinUp.end());

        auto hessianSpinDown = auxDM.getLocalDescriptors(
          DensityDescriptorDataAttributes::hessianSpinDown,
          std::make_pair(iPoint * 9, iPoint * 9 + 9));
        ddrho.insert(ddrho.end(),
                     hessianSpinDown.begin(),
                     hessianSpinDown.end());

        double sumofSquaresdrhoTotal = 0.0;
        for (int i = 0; i < 3; i++)
          {
            drhoTotal[iPoint * 3 + i] =
              gradValuesSpinUp[i] + gradValuesSpinDown[i];
            sumofSquaresdrhoTotal +=
              drhoTotal[iPoint * 3 + i] * drhoTotal[iPoint * 3 + i];
          }
        modDRhoTotal[iPoint] = std::sqrt(sumofSquaresdrhoTotal);

        for (int i = 0; i < 9; i++)
          ddrhoTotal[iPoint * 9 + i] = hessianSpinUp[i] + hessianSpinDown[i];
        rhoTotalLap[iPoint] = ddrhoTotal[iPoint * 9] +
                              ddrhoTotal[iPoint * 9 + 4] +
        ...
        for (int i = 0; i < 3; ++i)
          {
            for (int j = 0; j < 3; ++j)
              {
                v_vec[i] += ddrhoTotal[iPoint * 9 + i * 3 + j] *
                            drhoTotal[iPoint * 3 + j];
              }
          }
    ...
    auto FDSpacing =
      auxDM.getLocalDescriptors(DensityDescriptorDataAttributes::FDSpacing,
                                std::make_pair(0, nPoints));

    int  StencilDataSize = 3 * 2 * stencilOrder1D * 4;
    auto rhoStencilData =
      auxDM.getLocalDescriptors(DensityDescriptorDataAttributes::rhoStencilData,
    ...
    GGAHandler.getexcAndvxc(
      rho, drho, ddrho, sigma, exc_Base, vxc_Base, nPoints);
    ...
    if (d_NNLLMGGAPtr != nullptr)
      {
        ...
        d_NNLLMGGAPtr->evaluate_exc_vxc(rho,
                                        modDRhoTotal,
                                        rhoTotalLap,
                                        w_vec,
                                        drhoTotal,
