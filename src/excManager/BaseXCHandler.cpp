...
BaseXCHandler::BaseXCHandler(const std::string &XFunctionalName,
      ...
      if (nspin == 1)
        {
          if (xc_func_init(&func_X, XC_GGA_X_PBE, XC_UNPOLARIZED) != 0)
            std::cerr << "Failed to initialize unpolarized exchange functional."
                      << std::endl;
        }
      else
        {
          if (xc_func_init(&func_X, XC_GGA_X_PBE, XC_POLARIZED) != 0)
            std::cerr << "Failed to initialize polarized exchange functional."
                      << std::endl;
        }
    }
      ...
      if (nspin == 1)
        {
          if (xc_func_init(&func_C, XC_GGA_C_PBE, XC_UNPOLARIZED) != 0)
            std::cerr
              << "Failed to initialize unpolarized correlation functional."
              << std::endl;
        }
      else
        {
          if (xc_func_init(&func_C, XC_GGA_C_PBE, XC_POLARIZED) != 0)
            std::cerr
              << "Failed to initialize polarized correlation functional."
              << std::endl;
        }
    }
}
...
void
BaseXCHandler::getexcAndvxc(const std::vector<double> &rho,
                            ...
                            int                        nPoints)
{
  ...
  /*
  // Output results
  std::cout << "Exchange-correlation energy per particle: " << exc[0] << ", " <<
  exc[1] << std::endl;
...
std::vector<double>
BaseXCHandler::getv(const std::vector<double> &vrho,
  ...
  for (int iSigma = 0; iSigma < nSigma; ++iSigma)
    {
      for (int jSigma = iSigma; jSigma < nSigma; ++jSigma)
        {
          int offset =
            iSigma * nSigma - (iSigma - 1) * iSigma / 2 + (jSigma - iSigma);
      ...
      for (int iSigma = 0; iSigma < nSigma; iSigma++)
        {
          ...
          for (int iSpin = 0; iSpin < nspin; iSpin++)
            {
              for (int iPoint = 0; iPoint < nPoints; iPoint++)
                {
                  v[iPoint * nspin + alpha] -=
                    v2rhosigma[(iPoint * 6) + iSpin * nSigma + iSigma] *
                    vecvecDotproduct(drho,
                                     g,
                                     iPoint * nspin * 3 + iSpin * 3,
                                     iPoint * 3);
                }
            }
          ...
          for (int jSigma = 0; jSigma < nSigma; jSigma++)
            {
              ...
              for (int iPoint = 0; iPoint < nPoints; iPoint++)
                {
                  v[iPoint * nspin + alpha] -=
                    v2sigma2[iPoint * 6 + index] *
                    vecvecDotproduct(gradSigma,
                                     g,
                                     iPoint * nSigma * 3 + jSigma * 3,
                                     iPoint * 3);
                }
            }
          ...
          for (int iPoint = 0; iPoint < nPoints; iPoint++)
            v[iPoint * nspin + alpha] -=
              (vsigma[iPoint * 3 + iSigma] * divg[iPoint]);
        }
    }
  return v;
}
...
std::vector<double>
BaseXCHandler::getGradSigma(const std::vector<double> &drho,
                            const std::vector<double> &ddrho,
                            int                        nPoints,
                            int                        nSigma)
{
      ...
      for (int iSigma = 0; iSigma < nSigma; iSigma++)
        {
          ...
          if (iSigma == 0)
            {
              auto ddrho0drho0        = multiplyDDrhoDrho(ddrho,
                                                   drho,
                                                   (iPoint * nspin) * 9,
                                                   (iPoint * nspin) * 3);
            ...
            {
              auto ddrho0drho1        = multiplyDDrhoDrho(ddrho,
                                                   drho,
                                                   (iPoint * nspin) * 9,
                                                   (iPoint * nspin + 1) * 3);
              auto ddrho1drho0        = multiplyDDrhoDrho(ddrho,
                                                   drho,
                                                   (iPoint * nspin + 1) * 9,
                                                   (iPoint * nspin) * 3);
            ...
            {
              auto ddrho1drho1        = multiplyDDrhoDrho(ddrho,
                                                   drho,
                                                   (iPoint * nspin + 1) * 9,
                                                   (iPoint * nspin + 1) * 3);
          ...
          else
            {
              throw std::invalid_argument(
                "Invalid sigma index/nSigma passed to gradSigma");
            }
        }
    }
...
std::vector<double>
BaseXCHandler::getg(const std::vector<double> &drho,
                    int                        nPoints,
                    int                        ii,
                    int                        alpha)
{
...
std::vector<double>
BaseXCHandler::getDivg(const std::vector<double> &ddrho,
                       int                        nPoints,
                       int                        ii,
                       int                        alpha)
{
      ...
      for (int iPoint = 0; iPoint < nPoints; iPoint++)
        {
          ...
          divg[iPoint]    = ddrho[ddrho_index] + ddrho[ddrho_index + 4] +
                         ddrho[ddrho_index + 8];
        }
    }
  else if ((ii == 0 && alpha == 0) || (ii == 2 && alpha == 1))
      ...
      for (int iPoint = 0; iPoint < nPoints; iPoint++)
        {
          ...
          divg[iPoint]    = 2.0 * (ddrho[ddrho_index] + ddrho[ddrho_index + 4] +
                                ddrho[ddrho_index + 8]);
        }
    }
  return divg;
...
std::vector<double>
BaseXCHandler::multiplyDDrhoDrho(const std::vector<double> &ddrho,
                                 const std::vector<double> &drho,
                                 int                        ddrhoStartIndex,
                                 int                        drhoStartIndex)
{
  ...
  for (int i = 0; i < 3; i++)
    {
      ddrhodrho[i] =
        ddrho[ddrhoStartIndex + 3 * i] * drho[drhoStartIndex] +
...
double
BaseXCHandler::vecvecDotproduct(const std::vector<double> &vec1,
