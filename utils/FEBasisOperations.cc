...
//      std::cout<<" size of stiffness vec = "<<d_cellStiffnessMatrixBasisType.size()<<"\n";
    ...
    {
      distributedCPUVec<double> stiffnessVector, sqrtStiffnessVector, invStiffnessVector,
        invSqrtStiffnessVector;
      d_matrixFreeDataPtr->initialize_dof_vector(stiffnessVector, d_dofHandlerID);
        ...
        if (cell->is_locally_owned())
          {
            ...
            for (unsigned int iDoF = 0; iDoF < d_nDofsPerCell; ++iDoF)
              for (unsigned int iQuad = 0; iQuad < nQuadsPerCell; ++iQuad)
                stiffnessVectorLocal(iDoF) += fe_values.shape_grad(iDoF, iQuad) *
...
//              }
      ...
      for (dealii::types::global_dof_index i = 0; i < stiffnessVector.size(); ++i)
      ...
      dftfe::utils::MemoryStorage<ValueTypeBasisData,
                                  dftfe::utils::MemorySpace::HOST>
        cellStiffnessVectorHost, cellInvStiffnessVectorHost, cellInvSqrtStiffnessVectorHost,
              ...
              for (unsigned int iNode = 0; iNode < d_nDofsPerCell; ++iNode)
                {
                  ...
                  if ((*d_constraintsVector)[d_dofHandlerID]->is_constrained(
                        globalIndex))
                    {
                      cellStiffnessVectorHost[iElemCount * d_nDofsPerCell + iNode] =
                        1.0;
                      cellInvStiffnessVectorHost[iElemCount * d_nDofsPerCell +
                                            iNode]     = 1.0;
                      cellSqrtStiffnessVectorHost[iElemCount * d_nDofsPerCell +
                                             iNode]    = 1.0;
                      cellInvSqrtStiffnessVectorHost[iElemCount * d_nDofsPerCell +
                                                iNode] = 1.0;
                    }
                  else
                    {
                      ValueTypeBasisData stiffnessVecValue = stiffnessVector(globalIndex);
                      cellStiffnessVectorHost[iElemCount * d_nDofsPerCell + iNode] =
                        stiffnessVecValue;
                      cellSqrtStiffnessVectorHost[iElemCount * d_nDofsPerCell +
                                             iNode] = std::sqrt(stiffnessVecValue);
                      if (std::abs(stiffnessVecValue) > 1.0e-15)
                        {
                          cellInvStiffnessVectorHost[iElemCount * d_nDofsPerCell +
                                                iNode] = 1.0 / stiffnessVecValue;
      ...
      if (basisType)
        {
          ...
          d_cellInverseStiffnessVectorBasisType.resize(cellInvStiffnessVectorHost.size());
          d_cellInverseStiffnessVectorBasisType.copyFrom(cellInvStiffnessVectorHost);
          d_cellSqrtStiffnessVectorBasisType.resize(cellSqrtStiffnessVectorHost.size());
          d_cellSqrtStiffnessVectorBasisType.copyFrom(cellSqrtStiffnessVectorHost);
          ...
          d_inverseSqrtStiffnessVectorBasisType
            .template copyFrom<dftfe::utils::MemorySpace::HOST>(
              ...
              0,
              0);
          d_sqrtStiffnessVectorBasisType.resize(mpiPatternP2P->localOwnedSize() +
                                           mpiPatternP2P->localGhostSize());
          d_sqrtStiffnessVectorBasisType
            .template copyFrom<dftfe::utils::MemorySpace::HOST>(
              sqrtStiffnessVector.begin(), d_sqrtStiffnessVectorBasisType.size(), 0, 0);
          d_inverseStiffnessVectorBasisType.resize(mpiPatternP2P->localOwnedSize() +
                                              mpiPatternP2P->localGhostSize());
          d_inverseStiffnessVectorBasisType
            .template copyFrom<dftfe::utils::MemorySpace::HOST>(
              invStiffnessVector.begin(), d_inverseStiffnessVectorBasisType.size(), 0, 0);
      ...
      if (ceoffType)
        {
          ...
          if constexpr (!std::is_same<ValueTypeBasisCoeff,
                                      ValueTypeBasisData>::value)
            {
              ...
              d_cellStiffnessVectorCoeffType.resize(cellStiffnessVectorHost.size());
              d_BLASWrapperPtr->copyValueType1ArrToValueType2Arr(
                d_nDofsPerCell * d_nCells,
              ...
              d_sqrtStiffnessVectorCoeffType.resize(mpiPatternP2P->localOwnedSize() +
                                               mpiPatternP2P->localOwnedSize());
              d_BLASWrapperPtr->copyValueType1ArrToValueType2Arr(
                d_sqrtStiffnessVectorCoeffType.size(),
                d_sqrtStiffnessVectorBasisType.data(),
                d_sqrtStiffnessVectorCoeffType.data());
              d_stiffnessVectorCoeffType.resize(mpiPatternP2P->localOwnedSize() +
                                           mpiPatternP2P->localOwnedSize());
              d_BLASWrapperPtr->copyValueType1ArrToValueType2Arr(
